        ...
        <ul>
              ...
              <li><a class="nav-link scrollto" href="display_questions.php">Modify Questions</a></li>
					<li class="dropdown active"><a class="nav-link scrollto active" href="#"><span>Reports</span> <i class="bi bi-chevron-down"></i></a>
						...
						<li><a class="nav-link scrollto active" href="user_report.php">User Reports</a></li>
...
<?php
	$sql = "SELECT org_name,user_name,user_role,user_email,user_org_id FROM org_info JOIN user_info ON org_info.pk_org_id = user_info.user_org_id ORDER BY user_org_id, user_role";
	//$sql1 = "SELECT user_role, COUNT(user_role) AS role_count FROM user_info GROUP BY user_role UNION ALL SELECT 'Grand Total', COUNT(*) FROM user_info ORDER BY CASE WHEN user_role = 'Grand Total' THEN 1 ELSE 0 END, user_role";
	...
	if ($result->num_rows > 0) {
 	...
 	echo "<table cellspacing='10' cellpadding='10'>
            <tr><th>Organization Name</th><th>User Names</th><th>User Role</th><th>User Email</th></tr>";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($user_org_id != $current_org_id) {
            ...
            echo "<td class='merged-cell'></td>";  // Print an empty cell for repeated org_name
        }

        // Print user details
        echo "<td>". $row["user_name"]."</td><td>". $row["user_role"]."</td><td  class='roles'>". $row["user_email"]."</td>";
	...
	if ($count->num_rows > 0) {
 	echo "<table cellspacing ='10' cellpadding ='10'>";
	echo "<th>Total Roles</th><th>Number of Totals</th>";
    	while($row = $count->fetch_assoc()) {

        echo "<tr><td  class='roles'>". $row["user_role"]."</td><td>". $row["role_count"]."</td></tr>";
    	}	
	echo "</table>";
