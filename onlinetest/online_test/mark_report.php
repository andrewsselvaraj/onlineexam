        ...
        <ul>
              ...
              <li><a class="nav-link scrollto" href="display_questions.php">Modify Questions</a></li>
					<li class="dropdown"><a class="nav-link scrollto active" href="#"><span>Reports</span> <i class="bi bi-chevron-down"></i></a>
						<ul>
						<li><a class="nav-link scrollto" href="report.php">Total reports</a></li>
						<li><a href="user_report.php">User Reports</a></li>
						<li><a class="nav-link scrollto active" href="mark_report.php">Mark reports</a></li>
						</ul>
					 </li>
...
<?php
	$sql = "SELECT DISTINCT COUNT(*) AS user_mark, (SELECT COUNT(question_name) FROM question_info_v_two q WHERE q.fk_subject_id = ei.fk_subject_id) AS total_questions, si.subject_name, ci.class_name, ui.user_name FROM exam_info ei JOIN answer_info_v_two ai ON ei.fk_user_selected_answer_id = ai.pk_answer_id JOIN subject_info si ON ei.fk_subject_id = si.pk_subject_id JOIN class_info ci ON ei.fk_class_id = ci.pk_class_id JOIN user_info ui ON ei.fk_org_id = ui.user_org_id WHERE ai.correct_answer = 'Yes' GROUP BY ei.exam_session_id, ei.fk_subject_id, si.subject_name, ci.class_name, ui.user_name";
	...
	echo "<th>User Name</th><th>Class Name</th><th>Subject Names</th><th>Scored Marks</th><th>Total Marks</th>";
    while($row = $result->fetch_assoc()) {

        echo "<tr><td>". $row["user_name"]."</td><td>". $row["class_name"]."</td><td>". $row["subject_name"]."</td><td>". $row["user_mark"]."</td><td>". $row["total_questions"]."</td></tr>";
